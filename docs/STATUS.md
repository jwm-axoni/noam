...
product: Noam
date: 2026-07-13
tags: [thread, status, roadmap]
---

# Noam: Build Status

> Live checklist. Update as phases land. Back to index: [[Noam]].
...
- **Deployment:** 🟢 Production-ready (2026-07-15). The sync WebSocket is served on the HTTP
  ...
  `app/.railway/railway.ts` (Dockerfile build, pre-deploy migrations + healthcheck), and [[DEPLOY]].
  Noam is free and self-hosted only — no managed backend is operated by this project; desktop
  releases ship via `v*` tags → signed installers → Tauri updater.
...
- [x] Better Auth: accounts, email+password (argon2id), server-side sessions; token in OS keychain
  (Rust `keyring` crate, service `com.noam.app`; `lib/auth` + `lib/api`).
...
- [x] **Shareable note links.** `noam://note/<vault>/<doc>` via `tauri-plugin-deep-link`
...
- [x] **Password reset**: "Forgot password?" in the desktop dialog and on `/oauth/login` →
  ...
  Google-only account, so the same flow sets a first password) → `noam://signin` bounces back into
...
- [x] **Sign-up verification email** (soft): sent on sign-up, `emailVerified` set on click, lands on
  `/email-verified` → `noam://verified` → the app re-reads its session; Account settings shows
...
- [x] **Invitation emails** → `POST /api/invitations/:id/send` (explicit, reports sent/failed; not a
  ...
  `noam://invite/<id>?server=` deep link → the app signs in/up with the invited address and
