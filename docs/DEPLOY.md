...
```bash
docker build -f app/apps/server/Dockerfile -t noam-server .
```
...
docker run --rm \
  -e DATABASE_URL=postgres://context:context@your-postgres-host:5432/context \
  -e JWT_SECRET="$(openssl rand -base64 32)" \
  noam-server node dist/db/migrate.js
```
...
services:
  postgres:
    ...
    environment:
      ...
      POSTGRES_DB: context
    volumes:
      - thread-postgres:/var/lib/postgresql/data
    healthcheck:
...
3. Apply the checked-in settings from a clone. Needs the Railway CLI 5.42 or
   ...
   ```bash
   cd app
   export NOAM_GITHUB_REPOSITORY="neutral-owner/noam"
   ...
   volumes or domains. `NOAM_GITHUB_REPOSITORY` supplies the current neutral
   GitHub owner and repository name. A project whose name contains "staging"
   deploys the `staging` branch; anything else deploys `main`. The settings then live on the
...
[![Deploy on Railway](https://railway.com/button.svg)](https://railway.com/deploy/noam-server?utm_medium=integration&utm_source=button&utm_campaign=thread)

Template [**`noam-server`**](https://railway.com/deploy/noam-server) — two
...
| `thread` | Builds `app/apps/server/Dockerfile` from this repo, gets an HTTPS domain |
...
code (`/deploy/CZ25Mu`); publishing moves it to the vanity slug (`/deploy/noam-server`)
...
```bash
curl -sL https://railway.com/deploy/noam-server | grep -o '<title>[^<]*</title>'
...
`VITE_SERVER_URL` inlined by Vite. A future staging release process should set
that value from protected release configuration. See `docs/RELEASE.md`.
...
| `DEEP_LINK_SCHEME` | no | `thread` | URL scheme of the desktop app the server's pages bounce into. Set `noam-staging` on the server behind the Staging app so production and staging links open the right app on a machine that has both. |
| `EMAIL_FROM` | for email | unset | **Outbound email (optional).** Sender address, e.g. `Noam <no-reply@example.com>`. With this and ONE transport below, password reset ("Forgot password?"), sign-up verification and invitation emails switch on. Unset ⇒ email off and none of those is offered (invitations are shared as a link instead). |
...
which is clickable in chat (a bare `noam://` scheme is not) and bounces into
