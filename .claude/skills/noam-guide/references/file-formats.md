# What file types Noam supports

The single most common question. Answer it precisely, because "supports" means
four different things in Noam and people usually mean one of them:
...
| **Viewable file** | Shows in the sidebar, opens in a built-in viewer, cannot be edited inside Noam | Images: `.png` `.jpg` `.jpeg` `.gif` `.webp` `.svg` `.avif` · Documents: `.pdf` |
...
**Images and PDFs are view-only.** Noam previews them in place (images inline, PDFs as a
...
Noam copies it into `attachments/` at the vault root, names it by content hash (so the same
...
**Office files (DOCX, XLSX, PPTX) are attachments, not notes.** Noam does not open, convert,
...
launch an external app. To get the *content* into Noam, convert it to Markdown first (Pandoc,
...
**Code files (`.py`, `.js`, `.ts`, `.rs`, etc.) are deliberately ignored.** Noam is a notes
app, not a code editor. The exclusion exists so you can point Noam at a real project folder
...
> to your team as a link, but Noam cannot open or edit Office files; convert them to Markdown
...
> Yes. Point Noam at the folder. Your `.md` files, folders, `[[wikilinks]]` and tags work as-is.
