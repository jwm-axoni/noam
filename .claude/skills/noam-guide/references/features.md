...
Noam is a "team second brain". Three things that normally do not go together:

1. **Your notes are plain Markdown files on your disk.** No database lock-in. Open them in any
   editor, back them up, put them in Git. If Noam vanished tomorrow you would still have
   everything.
...
collaborative apps (Notion, Confluence) keep your data in their database. Noam bridges the two.
...
- A **vault** is a folder of notes. It can be **Local** (just a folder, no account), **Synced**
  ...
  that Noam downloads to your machine).
...
- Default location for new vaults: `~/Documents/Noam Vaults/<vault name>` (under Documents so it
...
- **Folding.** Hover the left edge of a heading, a list item, a callout or a code block and a
  ...
  back. Noam remembers what you folded per note, on this device, and restores it the next time
...
- **Properties** are the block of information at the very top of a Markdown file (what other
  apps call "frontmatter" or "YAML"). Noam shows it as a small table you fill in, one row per
...
- **If Noam cannot read the properties, it shows them as plain text and never rewrites them.**
...
- **Public links**: turn a note into a read-only web page anyone with the link can read. Revoke
  any time. **Private links** (`noam://note/...`) open a note for teammates who already have
...
- **Local agents** (Claude Code, Codex, any script): nothing to configure. They edit the `.md`
  files; Noam notices and syncs. A human typing and an AI rewriting the same note merge.
...
- Bring your own model. Noam ships no AI model, no API key requirement, and no chat panel.
...
- The macOS app is Developer-ID signed and notarized. Auto-updates on every platform are
  verified with Noam's own signing key.
...
- In-app updater is disabled until the new release repository and signing key
  are configured.
...
No public Noam license has been selected. See `LICENSING.md` for the current
project status and `NOTICE` for retained third-party attribution. The Noam name
is covered separately by `TRADEMARK.md`.
