...
/** Compatibility entry: open the unified dialog at the requested legacy tab. */
...
export type VaultSettingsSectionId = Extract<
  SettingsSectionId,
  | "general"
  | "vaults"
  | "members"
...
const SYNC_LOCKED_SECTIONS: ReadonlySet<VaultSettingsSectionId> = new Set([
  "members",
  "access",
  "mcp",
  "versioning",
...
export function VaultSettingsSection({
  ...
  section: VaultSettingsSectionId;
  isSynced: boolean;
  canManage: boolean;
  ...
  if (SYNC_LOCKED_SECTIONS.has(section) && !isSynced) {
    const labels: Record<VaultSettingsSectionId, string> = {
      general: "General",
      vaults: "Vaults",
      members: "Members",
      billing: "Billing",
    ...
    return <SyncGate label={labels[section]} onGoToSync={() => onNavigate("general")} />;
  }
  switch (section) {
    case "general":
      return (
    ...
    case "vaults":
      return <VaultsTab />;
    case "members":
      return <MembersTab canManage={canManage} />;
    case "billing":
      return <BillingTab canManage={canManage} isSynced={isSynced} />;
    case "access":
      return (
        <div data-setting-id="vault-access" tabIndex={-1}>
    ...
    <div className="general-settings">
      {isSynced ? (
        <>
          ...
          <div className="menu-row" data-setting-id="vault-sync" tabIndex={-1}>
        ...
        <>
          ...
          <div className="sync-promo" data-setting-id="vault-sync" tabIndex={-1}>
            ...
            <div className="row invite-bar" data-setting-id="vault-name" tabIndex={-1}>
              <input
      ...
      <div
        ...
        data-setting-id={isSynced ? "vault-name" : undefined}
        tabIndex={isSynced ? -1 : undefined}
      >
...
function FreezeRootRow({ canManage }: { canManage: boolean }) {
  ...
  return (
    <>
      ...
      <label className="menu-row toggle-row" data-setting-id="freeze-root" tabIndex={-1}>
...
function VaultsTab() {
  ...
  return (
    <div className="legacy-settings-section" data-setting-id="vault-list" tabIndex={-1}>
      {session && (
        <>
...
function MembersTab({ canManage }: { canManage: boolean }) {
  ...
  return (
    <div className="legacy-settings-section" data-setting-id="team-members" tabIndex={-1}>
      {canManage && code && (
        <div className="join-code-row" data-setting-id="join-code" tabIndex={-1}>
 ...
 * so billing remains reachable from a local folder.
 */
function BillingTab({ canManage, isSynced }: { canManage: boolean; isSynced: boolean }) {
  ...
  return (
    <div className="legacy-settings-section" data-setting-id="billing-plan" tabIndex={-1}>
      {renderVaultCard()}
...
function VersioningTab({ canManage }: { canManage: boolean }) {
  ...
  if (!vault) {
    return (
      <div className="muted perm-empty" data-setting-id="versioning" tabIndex={-1}>
        Checkpoints need an active vault — create or switch to one first.
      </div>
    );
  }

  return (
    <div className="versioning-tab" data-setting-id="checkpoints" tabIndex={-1}>
      <div className="muted">
      ...
      {confirming && (
        <ConfirmDialog
          title="Revert the entire vault?"
          confirmLabel="Revert vault"
          onCancel={() => setConfirming(null)}
          onConfirm={() => revert(confirming)}
        >
          <p>
...
function McpTab() {
  ...
  if (!hasVault) {
    return (
      <div className="muted perm-empty" data-setting-id="mcp-connections" tabIndex={-1}>
  ...
  return (
    <div className="legacy-settings-section">
      ...
      <div className="join-code-row" data-setting-id="mcp-endpoint" tabIndex={-1}>
...
function UpdatesTab() {
  ...
  return (
    <div className="updates-tab" data-setting-id="check-updates" tabIndex={-1}>
      <div className="menu-row">
...
function ImportExportTab() {
  ...
  if (!vault) {
    return (
      <div className="muted perm-empty" data-setting-id="import-export" tabIndex={-1}>
        Import and export need an active vault — create or switch to one first.
      </div>
    );
  }
  ...
  return (
    <div className="io-tab">
      <section className="io-section" data-setting-id="import-files" tabIndex={-1}>
      ...
      <section className="io-section" data-setting-id="export-vault" tabIndex={-1}>
