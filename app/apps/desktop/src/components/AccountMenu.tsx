...
import { resolveSettingsSection } from "./settings/settingsRegistry";
...
export function AccountMenu({ compact = false }: { compact?: boolean }) {
  ...
  // Activity-bar affordances share the existing account/vault surfaces rather
  // than mounting duplicate settings or switcher dialogs.
  useEffect(() => {
    ...
    const openSettings = (event: Event) => {
      ...
      const requested =
        detail && typeof detail === "object" && "section" in detail
          ? (detail as { section?: unknown }).section
          : detail;
      setOpen(false);
      setSettingsSection(resolveSettingsSection(requested));
    };
    window.addEventListener("noam:open-account-menu", openMenu);
    window.addEventListener("noam:open-settings", openSettings);
    return () => {
      window.removeEventListener("noam:open-account-menu", openMenu);
      window.removeEventListener("noam:open-settings", openSettings);
    };
  }, []);
...
function VaultRows({
  ...
  return (
    <>
      <div className="menu-label">Noam Vaults</div>
      {rows.map((row) => (
        <button
          key={row.key}
