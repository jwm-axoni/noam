 ...
 * stays distinguishable from "reachable but not Noam" — the two send the user
...
describe("ApiClient.health", () => {
  ...
  it("resolves for a 2xx JSON body with ok:true, probing the given base", async () => {
    ...
    await expect(api.health("https://notes.example.com/thread/")).resolves.toBeUndefined();
    // The candidate URL, not the client's current base — and no session token,
    // because the point is to test the address.
    expect(seen.url).toBe("https://notes.example.com/thread/health");
  ...
  it("rejects a 200 that isn't a Noam health body", async () => {
