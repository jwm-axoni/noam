...
describe("parseAccountLink", () => {
  it("accepts both foldings of the two links", () => {
    expect(parseAccountLink("noam://verified")).toBe("verified");
    expect(parseAccountLink("noam:///verified")).toBe("verified");
    expect(parseAccountLink("noam://signin")).toBe("signin");
    expect(parseAccountLink("noam:///signin/")).toBe("signin");
  });

  it("accepts the Staging app's scheme too", () => {
    expect(parseAccountLink("noam-staging://verified")).toBe("verified");
  });
