...
describe("share links", () => {
  ...
  it("accepts the host-folded form some platforms deliver", () => {
    ...
    expect(parseNoteLink("noam:///note/org_1/doc_1")).toEqual({
      orgId: "org_1",
      docId: "doc_1",
    });
  });
  ...
  it("builds a clickable https link when a server base is known", () => {
    // Chat apps linkify https; a bare noam:// scheme just sits there as text.
    // The server's /open/note page bounces the click into the app.
    const target = { orgId: "org_123", docId: "d0c-4bcd" };
    expect(buildNoteLink(target, "https://api.noam.io")).toBe(
      "https://api.noam.io/open/note/org_123/d0c-4bcd",
    );
    // A reverse-proxy sub-path prefix on the server URL is preserved.
    expect(buildNoteLink(target, "https://example.com/thread/")).toBe(
      "https://example.com/thread/open/note/org_123/d0c-4bcd",
    );
  ...
  it("parses the https form, host-agnostic (self-hosted servers mint links too)", () => {
    const target = { orgId: "org_123", docId: "d0c-4bcd" };
    expect(parseNoteLink(buildNoteLink(target, "https://api.noam.io"))).toEqual(target);
  ...
  it("rejects anything that isn't one of ours", () => {
    for (const url of [
      ...
      "noam://vault/org_1",
      "noam://note/org_1", // no doc id
      "noam://note", // no ids at all
      "not a url at all",
      "",
    ]) {
