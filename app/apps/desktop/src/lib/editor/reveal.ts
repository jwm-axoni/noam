...
import { isReading, viewMode } from "./viewMode";
...
/** `focused` + its tracker. Kept in the editor's stable base so presentation
 *  mode reconfiguration cannot forget focus; every reveal consumer reads the
 *  same field. */
...
/** A presentation-only reconfiguration can change reveal output by itself. */
export function viewModeChanged(u: ViewUpdate): boolean {
  return u.startState.facet(viewMode) !== u.state.facet(viewMode);
}
...
export function selectionTouches(state: EditorState): (from: number, to: number) => boolean {
  if (isReading(state) || !isFocused(state)) return () => false;
