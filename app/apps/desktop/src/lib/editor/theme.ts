...
export const editorThemeSpec: Record<string, Record<string, string>> = {
  "&": {
    ...
    fontSize: "var(--type-document-size)",
  },
  // The prose column: a calm sheet, generous padding, comfortable measure.
  ".cm-scroller": {
    fontFamily: "var(--font-body)",
    lineHeight: "var(--type-leading-document)",
    overflowX: "hidden",
  },
  ...
  // Reading mode keeps remote-cursor layers visible, but removes the local
  // editing affordances. The active-line clauses are defensive: the editor
  // currently does not install an active-line fill, and this keeps that true
  // if one is added to the shared extension set later.
  "&.cm-reading .cm-cursor, &.cm-reading .cm-dropCursor": {
    display: "none",
  },
  ...
  ".cm-callout-title": { fontWeight: "var(--type-weight-semibold)" },
  ".cm-callout-icon": {
    display: "inline-flex",
    alignItems: "center",
    gap: "var(--sp-2)",
  ...
  ".cm-foldPlaceholder": {
    ...
    padding: "0 0.55em",
    margin: "0 0.25em",
    fontSize: "var(--type-caption-relative)",
    cursor: "pointer",
  },
  ...
  ".cm-frontmatter span": {
    ...
    fontWeight: "var(--type-weight-regular)",
    color: "var(--text-secondary)",
  },
