...
async function embedImage(
  ...
  file: File,
  save: SaveAttachment,
  lifetimePlugin: ViewPlugin<PasteLifetime>,
  lifetime: PasteLifetime,
  editabilityEpoch: number,
) {
  try {
    const bytes = new Uint8Array(await file.arrayBuffer());
    const src = await save(bytes, extFor(file.type));
    // Saving the attachment may finish, but its Markdown insertion belongs to
    // the editable view session that initiated it. Destroying the view or
    // crossing any read-only boundary (even if later unlocked) cancels it.
    if (
      !lifetime.alive ||
      view.plugin(lifetimePlugin) !== lifetime ||
      lifetime.editabilityEpoch !== editabilityEpoch ||
      view.state.readOnly
    ) {
      return;
    }
...
export function smartPaste(save: SaveAttachment | undefined) {
  const lifetimePlugin = ViewPlugin.fromClass(PasteLifetime);
  const handlers = EditorView.domEventHandlers({
    paste(event, view) {
      ...
      if (img) {
        ...
        const lifetime = view.plugin(lifetimePlugin);
        if (lifetime) {
          void embedImage(
            view,
            img,
  ...
  return [lifetimePlugin, handlers];
}
