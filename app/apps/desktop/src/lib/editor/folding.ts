...
import { EditorState, StateField, type Extension, type Line, type StateEffect } from "@codemirror/state";
...
/**
 * Source is literal text, so its configuration contains neither fold state nor
 * fold decorations. This stable field remembers the visible mode's anchors at
 * the exact reconfiguration that removes `codeFolding()`.
 */
const rememberedFolds = StateField.define<readonly FoldAnchor[]>({
  create: () => [],
  update(value, tr) {
    const before = tr.startState.facet(viewMode);
    const after = tr.state.facet(viewMode);
    return before !== "source" && after === "source"
      ? foldAnchors(tr.startState)
      : value;
  },
});

/**
 * When folding is installed again, add restoration effects to that same mode
 * transaction. Keeping both the state field and fold keymap inside the
 * presentation compartment means `foldCode` can never append an uncontrolled
 * `codeFolding()` configuration while Source is active.
 */
const restoreRememberedFolds = EditorState.transactionExtender.of((tr) => {
  if (
    tr.startState.facet(viewMode) !== "source" ||
    tr.state.facet(viewMode) === "source"
  ) {
    return null;
  }
  const effects = foldEffectsFor(
    tr.state,
    tr.startState.field(rememberedFolds, false) ?? [],
  );
  return effects.length > 0 ? { effects } : null;
});

/** Stable memory for folds temporarily hidden by Source mode. */
export const preserveFoldsAcrossModes: Extension[] = [
  rememberedFolds,
  restoreRememberedFolds,
];
