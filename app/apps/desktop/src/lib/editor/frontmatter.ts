...
import { isReading, viewMode as editorViewMode } from "./viewMode";
 ...
 *
 * `source` wins whenever an editable caret is inside the region — the
 * code-fence rule, reused: a search hit there must show real YAML rather than
 * a panel that is silently read-only. Reading mode deliberately suppresses
 * that reveal. Source also wins when the YAML is outside the subset we can
 * edit, because a panel we cannot round-trip would be a panel that rewrites
 * someone's file.
 */
...
export function frontmatterView(state: EditorState): FrontmatterView {
  ...
  if (!fm) return "source";
  // Source mode is the literal document buffer. The Properties panel must not
  // become a second authority over these bytes while that mode is active.
  if (state.facet(editorViewMode) === "source") return "source";
  const mode = state.facet(propertiesMode);
  if (mode === "source") return "source";
  if (!isReading(state) && selectionInside(state.selection.ranges, fm)) return "source";
...
function buildFrontmatterDecorations(state: EditorState): DecorationSet {
  ...
  if (
    ...
    state.facet(editorViewMode) !== "source" &&
    (isReading(state) || !selectionTouches(state.selection.ranges, fm.from, fm.to))
  ) {
