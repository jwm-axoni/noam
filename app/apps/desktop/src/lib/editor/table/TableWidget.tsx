...
function dispatchChanges(view: EditorView, changes: SpanChange[]): void {
  // Widgets and portalled menus can outlive the transaction that locked the
  // editor. The current state, not the render-time prop, is authoritative.
...
function EditableTable({
  ...
  // A mode/access transition can happen without a click outside the widget.
  // Drop the uncommitted draft and close its portalled menu immediately.
  useEffect(() => {
    if (!readOnly) return;
    if (editing) handledCell.current = `${editing.row}:${editing.col}`;
    setEditing(null);
    setMenu(null);
  }, [readOnly, editing]);
  ...
  const startEdit = useCallback(
    (r: number, c: number, caret: number | null) => {
      if (readOnly || view.state.readOnly) return;
    ...
    [readOnly, view, cellRaw, editing, draft, commit],
  );
  ...
  const cellBody = (r: number, c: number) => {
    ...
    return (
      <input
        ...
        value={draft}
        readOnly={readOnly || view.state.readOnly}
        spellCheck
