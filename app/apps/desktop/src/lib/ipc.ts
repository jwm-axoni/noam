...
/** Effective managed vaults root (auto-initialized to ~/Noam on first call). */
...
export const getGraphEdgesFor = (noteIds: string[], expectedEpoch?: VaultEpoch) =>
  invoke<{ source: string; target: string }[]>("graph_edges_for", {
    noteIds,
    expectedEpoch: expectedEpoch ?? null,
  });
export const getNoteMeta = (path: string, expectedEpoch?: VaultEpoch) =>
  invoke<NoteMeta | null>("get_note_meta", { path, expectedEpoch: expectedEpoch ?? null });
...
/** Graph metadata from the SQLite index, including normalized frontmatter type. */
export const listGraphNodes = (expectedEpoch?: VaultEpoch) =>
  invoke<GraphNodeMeta[]>("list_graph_nodes", { expectedEpoch: expectedEpoch ?? null });
