// Receiving end of the app's `noam://` links: a shared note (`shareLink.ts`),
...
async function handle(urls: string[] | null): Promise<void> {
  ...
  // Account hand-offs (`noam://verified`, `noam://signin`) carry no data;
...
/**
 * Start listening for `noam://` opens. Returns a cleanup for React's effect.
 *
