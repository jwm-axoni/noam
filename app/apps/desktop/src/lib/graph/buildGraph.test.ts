...
describe("assembleGraph", () => {
  ...
  it("normalizes and carries indexed frontmatter type", () => {
    const graph = assembleGraph([title("a", "a.md", "A", " Project ")], []);
...
describe("applyGraphDelta (#83)", () => {
  const titles = [
    { id: "a", path: "A.md", title: "A", type: "project" },
    { id: "b", path: "B.md", title: "B", type: null },
    { id: "c", path: "C.md", title: "C", type: "person" },
  ];
  const base = () =>
  ...
  it("replaces a changed note's edges and refreshes its title without touching the rest", () => {
    ...
    const next = applyGraphDelta(base(), {
      nodes: [{ id: "a", path: "A.md", title: "Alpha", type: "meeting" }],
    ...
    expect(byId.get("a")).toMatchObject({ title: "Alpha", type: "meeting", linkCount: 1 });
    expect(byId.get("b")).toMatchObject({ title: "B", type: null, linkCount: 1 }); // lost a→b
  ...
  it("keeps edges INTO a changed note only if the delta still reports them", () => {
    ...
    const next = applyGraphDelta(base(), {
      nodes: [{ id: "b", path: "B.md", title: "B", type: null }],
  ...
  it("refuses a delta naming a note the graph does not have (caller rebuilds)", () => {
    expect(
      applyGraphDelta(base(), {
        nodes: [{ id: "new", path: "New.md", title: "New", type: null }],
        edges: [],
      }),
    ).toBeNull();
  });
  ...
  it("is a no-op for a delta that changes nothing", () => {
    ...
    const next = applyGraphDelta(g, {
      nodes: [{ id: "a", path: "A.md", title: "A", type: "project" }],
      edges: [{ source: "a", target: "b" }],
    });
