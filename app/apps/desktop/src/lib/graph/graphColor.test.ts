...
describe("assignColors — type", () => {
  ...
  it("uses stable token colors and a neutral unknown category", () => {
    const nodes = [
      { ...node("meeting", 0), type: "meeting" },
      { ...node("missing", 0), type: null },
      { ...node("invalid", 0), type: null },
    ];
    const { colorById, legend } = assignColors(nodes, "type", "#7f73ff");
    expect(colorById.get("meeting")).not.toBe(colorById.get("missing"));
    expect(colorById.get("missing")).toBe(colorById.get("invalid"));
    expect(legend).toContainEqual(expect.objectContaining({
      label: "Unknown / missing type",
      count: 2,
    }));
  });

  it("hashes custom types deterministically", () => {
    const custom = { ...node("custom", 0), type: "field-note" };
    expect(assignColors([custom], "type", "#fff").colorById.get("custom"))
      .toBe(assignColors([custom], "type", "#000").colorById.get("custom"));
  });
});
