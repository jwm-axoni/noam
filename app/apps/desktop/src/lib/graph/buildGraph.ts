...
export interface GraphNode {
  ...
  title: string;
  path: string;
  /** Normalized frontmatter type; null covers missing and invalid values. */
  type: string | null;
...
export function assembleGraph(nodesInput: GraphNodeMeta[], rawEdges: GraphEdge[]): Graph {
  ...
  const nodes: GraphNode[] = nodesInput.map((t) => ({
    ...
    path: t.path,
    type: normalizeGraphType(t.type),
...
export async function buildGraph(expectedEpoch?: number): Promise<Graph> {
  const [nodes, edges] = await Promise.all([
    listGraphNodes(expectedEpoch),
    getGraphEdges(expectedEpoch),
  ]);
  return assembleGraph(nodes, edges);
}
...
export function applyGraphDelta(graph: Graph, delta: GraphDelta): Graph | null {
  ...
  const nodes: GraphNodeMeta[] = graph.nodes.map((n) => {
    const f = fresh.get(n.id);
    return f ?? { id: n.id, path: n.path, title: n.title, type: n.type };
  });
  ...
  return assembleGraph(nodes, [...kept, ...delta.edges]);
}
 ...
 */
export async function fetchGraphDelta(
  paths: string[],
  expectedEpoch?: number,
): Promise<GraphDelta | null> {
  const metas = await Promise.all(paths.map((p) => getNoteMeta(p, expectedEpoch)));
...
export function normalizeGraphType(value: unknown): string | null {
