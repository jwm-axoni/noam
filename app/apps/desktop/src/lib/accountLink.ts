// The two data-less `noam://` links the server's account pages bounce into
// (routes/account-pages.ts): `noam://verified` after the email-confirmation
// link is clicked, and `noam://signin` after a password was reset in the
...
// deliver `noam://verified` with host "verified", others `noam:///verified`
// with it in the path.
