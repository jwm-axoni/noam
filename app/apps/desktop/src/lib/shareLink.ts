// Shareable note links — `noam://note/<orgId>/<docId>`.
//
 ...
 * `/open/note` page bounces straight into the `noam://` deep link. Without a
...
export function parseNoteLink(url: string): NoteLinkTarget | null {
  ...
  // `noam://note/<org>/<doc>` parses with host "note" and pathname
