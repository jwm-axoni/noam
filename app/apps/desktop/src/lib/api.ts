 ...
 * up; `not-noam` means the address is fine and something else is answering on
 * it (a proxy's default page, a different app, the wrong port).
 */
export class ServerCheckError extends Error {
  constructor(
    public kind: "unreachable" | "not-noam",
    message: string,
  ) {
    super(message);
...
const NOT_NOAM_MESSAGE = "That address answered, but it isn't a Noam server.";
...
export const ORIGIN_HEADER = "x-noam-origin";
...
export class ApiClient {
  ...
  /**
   * This app instance's opaque id. Sent on every HTTP call as `x-noam-origin`
  ...
  /**
   * Is there a Noam server at this address? Resolves if yes, throws a
  ...
  async health(baseUrl?: string): Promise<void> {
    ...
    if (!res.ok) throw new ServerCheckError("not-noam", NOT_NOAM_MESSAGE);
    const text = await res.text();
    let parsed: unknown;
    try {
      parsed = JSON.parse(text);
      ...
      throw new ServerCheckError("not-noam", NOT_NOAM_MESSAGE);
    }
    const ok =
      !!parsed && typeof parsed === "object" && (parsed as { ok?: unknown }).ok === true;
    if (!ok) throw new ServerCheckError("not-noam", NOT_NOAM_MESSAGE);
  }
