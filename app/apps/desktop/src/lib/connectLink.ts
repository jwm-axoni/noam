// Invite links that point the app at a server — `noam://connect?server=<url>`.
//
 ...
 *
 * Both foldings are accepted: `noam://connect?…` parses with host "connect"
 * and an empty path, while some platforms hand the URL over as
 * `noam:///connect?…` with the host folded into the path instead. Depending
