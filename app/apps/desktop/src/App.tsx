...
import { requestSearchInputFocus } from "./components/searchFocus";
...
/** Per-note presentation choice in the main note-tool cluster. */
function ViewModeSelector() {
  const path = useStore((state) => state.openNote?.path ?? null);
    ...
    >
      {VIEW_MODE_LABELS.map((option) => (
        <button
          ...
          title={`${option.label} view (⌘E to cycle)`}
          aria-pressed={mode === option.mode}
          onClick={() => useStore.getState().setViewMode(option.mode)}
        >
          {option.label}
        </button>
      ))}
 ...
 * arriving as a `noam://` deep link while the app is signed out:
 *
...
export default function App() {
  ...
  // available. Losing availability closes the orphaned tool tab as well.
  useEffect(() => {
    const found = findPanelTab(useLayoutStore.getState().layout, "history");
    if (!found) return;
    if (versionDocId) void useStore.getState().openVersionPanel(versionDocId);
    else closeHistoryPanel();
  }, [versionDocId, historyPanelPresent]);
  ...
  const onPanelOpen = (type: PanelType) => {
    if (type === "history" && versionDocId) {
      void useStore.getState().openVersionPanel(versionDocId);
    }
  };

  const onPanelClose = (type: PanelType) => {
    if (type === "history") useStore.getState().closeVersionPanel();
  };

  // `noam://` links, from a teammate's chat window into this app. Mounted for
  ...
  // Window commands: ⌘N new note, ⌘W close tab, Ctrl-Tab cycle,
  // ⌘E view mode, ⌘S/⌘G/⌘F, reload.
  useEffect(() => {
    ...
    const onKey = async (e: KeyboardEvent) => {
      ...
      if (e.ctrlKey && !e.metaKey && !e.altKey && e.key === "Tab") {
        ...
        const i = group.tabs.findIndex((tab) => tab.id === group.activeTabId);
        const step = e.shiftKey ? -1 : 1;
        const next = group.tabs[(i + step + group.tabs.length) % group.tabs.length];
        if (next?.kind === "note") void useStore.getState().openNoteByPath(next.path);
      ...
      if (e.altKey && !e.metaKey && !e.ctrlKey && (e.key === "ArrowLeft" || e.key === "ArrowRight")) {
        const path = e.key === "ArrowLeft" ? navigationHistory.back() : navigationHistory.forward();
        if (path) {
          e.preventDefault();
      ...
      if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === "f") {
        ...
        if (found) {
          ...
          useLayoutStore.getState().dispatch({ type: "open-panel", panelType: "search", zone: "left" });
        }
        requestSearchInputFocus();
        window.requestAnimationFrame(() => {
          document.querySelector<HTMLInputElement>('[data-panel-type="search"] .search-box')?.focus();
        });
      }
    ...
    <div className="app-shell">
      ...
      <div className="app">
        <WorkspaceShell
          ...
          historyAvailable={!!versionDocId && !isPreview}
          ...
          <header className="main-header">
            ...
            <button
              ...
              onClick={() => useLayoutStore.getState().dispatch({
                type: "open-panel",
                panelType: "graph",
                zone: "center",
                groupId: CENTER_NOTE_GROUP_ID,
