//! Data-volume stress benchmark for the Noam local core.
//!
...
//!   NOAM_BENCH_NOTES       total notes to generate         (default 2000)
//!   NOAM_BENCH_LINKS       wikilinks emitted per note      (default 8)
//!   NOAM_BENCH_LARGE_EVERY every Nth note is a "large" one  (default 50)
//!   NOAM_BENCH_LARGE_KB    approx size of a large note (KB) (default 64)
...
fn data_volume_bench() {
    let notes = env_usize("NOAM_BENCH_NOTES", 2000);
    let links_per = env_usize("NOAM_BENCH_LINKS", 8);
    let large_every = env_usize("NOAM_BENCH_LARGE_EVERY", 50);
    let large_kb = env_usize("NOAM_BENCH_LARGE_KB", 64);

    let vault = std::env::temp_dir().join(format!("thread-bench-{notes}"));

    println!("\n=== Noam data-volume benchmark ===");
    println!(
    ...
    let batch_n = env_usize("NOAM_BENCH_BATCH", 200).min(notes);
    ...
    let per_file_n = batch_n.min(env_usize("NOAM_BENCH_PER_FILE", 50));
    let t = Instant::now();
    ...
    let drop_n = env_usize("NOAM_BENCH_DROP", 1000);
