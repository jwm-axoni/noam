...
/// Build the full callback page. `ok` selects the accent + status pip; the Noam
/// Fan symbol is the mark. `title`/`body` are static copy (never user-supplied), so
...
fn render_page(ok: bool, title: &str, body: &str) -> String {
    ...
    format!(
        "<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\">\
         ...
         <title>{title} · Noam</title><style>{css}</style></head>\
         <body><main class=\"card\">\
         <img class=\"wordmark\" src=\"{wordmark}\" alt=\"Noam\">\
...
fn handle_connection(mut stream: std::net::TcpStream, expected_state: &str) -> OauthResult {
    ...
    let (ok, title, body, result) = match (&code, &error) {
        (Some(c), _) => (
            true,
            "Signed in",
            "You're all set — close this tab and head back to Noam.",
            Ok(c.clone()),
        ),
        (None, Some(e)) => (
            false,
