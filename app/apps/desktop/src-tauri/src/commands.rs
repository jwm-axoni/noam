...
const DEFAULT_ROOT_DIR_NAME: &str = "Noam Vaults";

/// Default managed root: `<home>/Documents/Noam Vaults`. Lives under Documents
...
/// Graph node metadata in one epoch-pinned indexed batch. `Index` extracts the
/// note type from its stored frontmatter JSON and never reads markdown here.
#[tauri::command]
...
mod tests {
    ...
    fn vault_info_names_filesystem_roots() {
        ...
        let v = vault_info(Path::new("/vaults/Notes"), 1);
