...
//! keychain under the service `com.noam.app`, keyed by a caller-supplied
...
/// Keychain "service" namespace for all Noam secrets.
///
/// Set once at the Phase-0 identifier cleanup (2026-09-15): the fork must not
/// share the predecessor app's keychain namespace with an existing install,
/// and the service name carries the claimed mark. Do not change it again —
/// changing it would orphan every user's already-stored keychain entries
/// (precedent: Slack still ships under `com.tinyspeck.*`).
pub const SERVICE: &str = "com.noam.app";
