...
/// separate from `NoteTitle` avoids making autocomplete/registry callers pay
/// for frontmatter parsing they do not use.
...
impl Index {
    ...
    pub fn resolve_wikilink(&self, name: &str) -> AppResult<Option<ResolvedLink>> {
        ...
        // 1. Full relative path (e.g. "Projects/Noam").
    ...
    /// Every graph node from the derived SQLite index in one pass. This reads
    /// the JSON already stored in `notes.frontmatter`; it never re-opens the
    /// markdown files when the graph is shown.
    pub fn list_graph_nodes(&self) -> AppResult<Vec<GraphNode>> {
        let mut stmt = self
            .conn
            .prepare("SELECT id, path, title, frontmatter FROM notes ORDER BY title")?;
        let rows = stmt.query_map([], |r| {
            let frontmatter = r.get::<_, Option<String>>(3)?;
            Ok(GraphNode {
                id: r.get(0)?,
                path: r.get(1)?,
                title: r.get::<_, Option<String>>(2)?.unwrap_or_default(),
...
mod tests {
    ...
    #[test]
    fn list_graph_nodes_reads_normalized_type_from_indexed_frontmatter() {
        let tmp = tempfile::tempdir().unwrap();
        let vault = tmp.path().to_path_buf();
        write_note(&vault, "Meeting.md", "---\ntype: ' Meeting '\n---\n# Meeting").unwrap();
        write_note(&vault, "Invalid.md", "---\ntype: [project]\n---\n# Invalid").unwrap();
        write_note(&vault, "Missing.md", "# Missing").unwrap();
        let idx = Index::open(&vault).unwrap();
        idx.rebuild(&vault).unwrap();

        let nodes = idx.list_graph_nodes().unwrap();
        let types: HashMap<_, _> = nodes
            .into_iter()
            .map(|node| (node.path, node.r#type))
            .collect();
        assert_eq!(types.get("Meeting.md"), Some(&Some("meeting".to_string())));
        assert_eq!(types.get("Invalid.md"), Some(&None));
        assert_eq!(types.get("Missing.md"), Some(&None));

        // The command reads current index state rather than re-opening Markdown
        // itself. Reindexing one changed note updates the next batch result.
        write_note(&vault, "Meeting.md", "---\ntype: Project\n---\n# Meeting").unwrap();
        idx.index_note(&vault, &vault.join("Meeting.md")).unwrap();
        let updated = idx
            .list_graph_nodes()
            .unwrap()
            .into_iter()
            .find(|node| node.path == "Meeting.md")
            .unwrap();
