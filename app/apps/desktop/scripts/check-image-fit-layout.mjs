...
function waitForServer(attempts = 50) {
  return new Promise((resolve, reject) => {
    const poll = (remaining) => {
      const call = request(`http://127.0.0.1:${port}/src/components/filePreview/imageFitLayout.fixture.html`, { method: "HEAD" }, (response) => {
        response.resume();
...
try {
  ...
  const browser = spawn(chrome, [
    ...
    `http://127.0.0.1:${port}/src/components/filePreview/imageFitLayout.fixture.html`,
  ], { stdio: ["ignore", "pipe", "pipe"] });
  let html = "";
  let errors = "";
