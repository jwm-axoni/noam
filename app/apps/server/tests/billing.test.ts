...
describe("billing", () => {
  ...
  describe("success page", () => {
    it("confirms a succeeded checkout with the provider and grants Pro before any webhook", async () => {
      ...
      expect(html).toContain(`noam://billing/upgraded?org=${encodeURIComponent(org.id)}`);
    ...
    it("writes nothing for a checkout that has not succeeded", async () => {
      ...
      expect(await res.text()).toContain(`noam://billing/upgraded?org=${encodeURIComponent(org.id)}`);
    ...
    it("renders for an unknown, malformed or missing checkout id (never a dead end)", async () => {
      for (const path of [
        ...
        expect(res.status).toBe(200);
        const html = await res.text();
        expect(html).toContain('href="noam://billing/upgraded"');
    ...
    it("still renders when the provider is down", async () => {
      ...
      expect(res.status).toBe(200);
      expect(await res.text()).toContain("noam://billing/upgraded");
    });
