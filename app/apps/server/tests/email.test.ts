...
describe("resolveEmailConfig", () => {
  ...
  it("picks Resend from its key, SMTP from its URL", () => {
    expect(
      resolveEmailConfig({ ...off, emailFrom: "Noam <no-reply@x.io>", resendApiKey: "re_1" }),
    ).toMatchObject({ transport: "resend", from: "Noam <no-reply@x.io>" });
    expect(
...
describe("transports", () => {
  ...
  it("resend transport posts the message and surfaces API failures", async () => {
    ...
    const m = resendMailer("Noam <no-reply@x.io>", "re_key", okFetch);
