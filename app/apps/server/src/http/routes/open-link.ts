 ...
 * `GET /open/connect`.
 *
 * A `noam://` link is dead weight in most chat apps: schemes nobody has
 ...
 * immediately bounces to the `noam://` deep link (which the OS hands to the
 ...
 * `noam://connect?server=<this server>`, which opens the app on a confirm
 * step.
 *
...
/** A proxy path prefix we're willing to keep, e.g. `/noam`. */
 ...
 * no different from mailing a `noam://connect` link directly — which the app
...
export function publicBaseUrl(c: Context): string {
  ...
  // A path prefix belongs in the URL — a server behind `example.com/noam` is
  ...
  // `/noam/open/connect` is a 404 — the same limitation `/open/note` has).
