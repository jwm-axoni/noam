...
function deepLinkScheme(): string {
  ...
  if (!/^[a-z][a-z0-9+.-]{0,63}$/.test(v)) {
    throw new Error(`DEEP_LINK_SCHEME must be a URL scheme like "noam" (got "${v}")`);
  }
  return v;
}
...
export const config = {
   ...
   * registers `thread`; the Staging app registers `noam-staging`, so a staging
   * server must say so or its links open whichever app grabbed `noam://` last
   * on a machine that has both installed.
   */
  deepLinkScheme: deepLinkScheme(),
  // ---- Outbound email (issue #99) ----
  /** Sender address, e.g. `Noam <no-reply@example.com>`. Required to send
