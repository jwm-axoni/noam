...
const fanDataUri = (colors: FanColors, viewBox: string, tile = false): string => {
  const tileSvg = tile
    ? `<rect width="64" height="64" rx="14" fill="#f2ecdf"/>`
    : "";
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${viewBox}"><title>Noam</title>${tileSvg}<rect x="21" y="7" width="30" height="42" rx="8" transform="rotate(12 36 28)" fill="${colors.back}"/><rect x="11" y="16" width="30" height="42" rx="8" fill="${colors.front}" stroke="${colors.gap}" stroke-width="6" paint-order="stroke"/></svg>`;
  return `data:image/svg+xml,${encodeURIComponent(svg)}`;
};

export const WORDMARK_LIGHT_DATA_URI = fanDataUri(
  { back: "#a09585", front: "#2b2724", gap: "#f2f1f6" },
  "-80 0 224 64",
);

export const WORDMARK_DARK_DATA_URI = fanDataUri(
  { back: "#8a8074", front: "#ebe4d6", gap: "#14141a" },
  "-80 0 224 64",
);

/** Dark-card default used by the server's shared account and OAuth shell. */
export const WORDMARK_DATA_URI = WORDMARK_DARK_DATA_URI;
