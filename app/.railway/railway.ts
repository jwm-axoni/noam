// Railway Infrastructure as Code for the Noam server service.
//
...
//
// Scope. `partial` limits the file to the `noam-server` service; Postgres,
...
export const partial = "noam-server";

const githubRepository = process.env.NOAM_GITHUB_REPOSITORY;
if (!githubRepository || !/^[^/\s]+\/[^/\s]+$/.test(githubRepository)) {
  throw new Error("Set NOAM_GITHUB_REPOSITORY to the current owner/repository name");
}
...
const SERVER_ENV = [
  ...
  "RESEND_API_KEY",
  "EMAIL_TRANSPORT",
  // Desktop URL scheme the server's pages bounce into; `noam-staging` on the
...
export default defineRailway((ctx) => {
  ...
  const server = service("noam-server", {
    source: github(githubRepository, { branch }),
    build: {
      builder: "DOCKERFILE",
